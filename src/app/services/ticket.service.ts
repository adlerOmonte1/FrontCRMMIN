...
export class TicketService {
  ...
  /** Opciones para un <select> (usadas por GastoViaje al elegir a qué ticket corresponde el gasto). */
  listConLabel(): Observable<OpcionSelect[]> {
    return this.listAll().pipe(
      map((tickets) =>
        tickets.map((t) => ({ id: t.id, label: `Ticket #${t.id} — peso bruto ${t.pesoBruto}` })),
      ),
    );
  }
}
